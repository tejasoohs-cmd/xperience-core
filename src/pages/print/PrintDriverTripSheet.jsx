...
import { formatDate, formatConfNumber } from '@/lib/formatters';
...
export default function PrintDriverTripSheet() {
  const { id } = useParams();
  ...
  if (!booking) return <div style={{ padding: 40, fontFamily: 'Arial' }}>Loading...</div>;
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', fontSize: 11, color: '#000', background: '#fff', maxWidth: 800, margin: '0 auto', padding: 20 }}>
        ...
        <tbody>
          <tr>
            ...
            <td style={{ width: '33%', verticalAlign: 'top', border: '1px solid #ccc', padding: 6 }}>
              ...
              <div style={{ fontSize: 9, color: '#666' }}>Arr. By: <span style={{ color: '#000' }}>Not Specified</span></div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
        <tbody>
          ...
          <tr>
            ...
            <td style={{ border: '1px solid #ccc', padding: 6, textAlign: 'center' }}>{vehicle?.plate_number || 'Unassigned'}</td>
            <td style={{ border: '1px solid #ccc', padding: 6, textAlign: 'center' }}>{driver?.name || 'Unassigned'}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
        <thead>
          <tr style={{ background: '#1f2937' }}>
            <th colSpan={2} style={{ padding: 6, color: '#fff', textAlign: 'left', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1 }}>Passenger & Routing Information</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td style={{ border: '1px solid #ccc', padding: 5, width: 120, fontSize: 9, color: '#555', verticalAlign: 'top' }}>Passenger:</td>
            <td style={{ border: '1px solid #ccc', padding: 5, fontWeight: 'bold' }}>{booking.primary_passenger_name || '—'}</td>
          </tr>
          {(booking.additional_passengers || []).length > 0 && (
            <tr>
              <td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, color: '#555' }}>Addt's Pax:</td>
              <td style={{ border: '1px solid #ccc', padding: 5 }}>{booking.additional_passengers.join(', ')}</td>
            </tr>
          )}
          {booking.flight_number && (
            <tr>
              <td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, color: '#555' }}>Flight #:</td>
              <td style={{ border: '1px solid #ccc', padding: 5, fontFamily: 'monospace' }}>{booking.flight_number} {booking.flight_schedule_time ? `(Sched: ${booking.flight_schedule_time})` : ''} {booking.flight_actual_time ? `(Actual: ${booking.flight_actual_time})` : ''}</td>
            </tr>
          )}
          {pts.map((pt, i) => (
            <React.Fragment key={i}>
              ...
              {pt.notes && (
                <tr>
                  <td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, color: '#666' }}>Remarks:</td>
                  <td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, fontStyle: 'italic', color: '#c47a00' }}>{pt.notes}</td>
                </tr>
              )}
            </React.Fragment>
          ))}
      ...
      {/* Payment — Driver sees N/A */}
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
        <tbody>
          <tr style={{ background: '#f3f4f6' }}>
            <th style={{ border: '1px solid #ccc', padding: 4, fontSize: 9, textTransform: 'uppercase', width: '50%', textAlign: 'left' }}>Pmt Type</th>
            <th style={{ border: '1px solid #ccc', padding: 4, fontSize: 9, textTransform: 'uppercase', textAlign: 'left' }}>Status</th>
          </tr>
          <tr>
            <td style={{ border: '1px solid #ccc', padding: 5 }}>N/A</td>
            <td style={{ border: '1px solid #ccc', padding: 5 }}>N/A</td>
          </tr>
        </tbody>
      </table>
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
        <tbody>
          ...
          <tr><td style={{ border: '1px solid #ccc', padding: 5 }}>N/A</td></tr>
        </tbody>
      </table>

      {/* Notes */}
      {(booking.trip_notes || booking.dispatch_notes || booking.greeting_sign) && (
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
          <thead><tr style={{ background: '#1f2937' }}><th colSpan={2} style={{ padding: 6, color: '#fff', textAlign: 'left', fontSize: 10, textTransform: 'uppercase' }}>Notes / Comments</th></tr></thead>
          <tbody>
            {booking.greeting_sign && <tr><td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, color: '#555', width: 120 }}>Greeting Sign:</td><td style={{ border: '1px solid #ccc', padding: 5, fontWeight: 'bold' }}>{booking.greeting_sign}</td></tr>}
            {booking.trip_notes && <tr><td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, color: '#555' }}>Trip Notes:</td><td style={{ border: '1px solid #ccc', padding: 5 }}>{booking.trip_notes}</td></tr>}
            {booking.dispatch_notes && <tr><td style={{ border: '1px solid #ccc', padding: 5, fontSize: 9, color: '#555' }}>Dispatch Notes:</td><td style={{ border: '1px solid #ccc', padding: 5 }}>{booking.dispatch_notes}</td></tr>}
          </tbody>
        </table>
      )}

      {/* Footer — Driver Fill-in */}
      <div style={{ border: '2px dashed #999', padding: 10, marginTop: 16 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, fontSize: 10 }}>
          <span>G-out: ________</span>
          <span>Arr Tm: ________</span>
          <span>Pax Load: ________</span>
          <span>Pax Drop: ________</span>
          <span>G-in: ________</span>
          <span>Mls-out: ________</span>
          <span>Mls-in: ________</span>
        </div>
      </div>
    </div>
  );
